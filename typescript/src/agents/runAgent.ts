...
import { ToolCallBudget, loadAgentRole, loadPrompt } from "../harness/index.js";
...
export async function runAgent(params: RunAgentParams): Promise<AgentResult> {
  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    ...
    if (result.stopReason !== "tool_use") {
      ...
      return { taskId: params.taskId, role: role.role, text: textOf(result.message), artifactRefs, status: "ok" };
    }
    ...
    for (const call of toolCalls) {
      ...
      try {
        budget.consume();
        const output = await params.harness.execute(
          role,
          { idempotencyKey: call.id, toolName: call.name, input: call.input, delegationDepth: params.delegationDepth },
          params.runtime,
        );
        params.tracer.endSpan(toolSpan, "ok");
        if (call.name === "write_artifact") artifactRefs.push(output as AgentResult["artifactRefs"][number]);
        toolResults.push({ type: "tool_result", toolCallId: call.id, output });
      } catch (error) {
        params.tracer.endSpan(toolSpan, "error", { attributes: { error: String(error) } });
        toolResults.push({ type: "tool_result", toolCallId: call.id, output: { message: String(error) }, isError: true });
      }
    }
